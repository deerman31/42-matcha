...
import { setAccessToken } from "~/session.server";
...
export const action: ActionFunction = async ({ request }) => {
  ...
  try {
    const response = await fetch("http://back:3000/api/login", {
      method: "POST",
    ...
    if (!response.ok) {
      return new Response(
        JSON.stringify({
          formError: responseData.error || "Login中にエラーが発生しました。",
          fields,
        }),
        {
    ...
    // セッションにトークンを保存
    const session = await setAccessToken(request, responseData.access_token);

    if (responseData.is_preparation) {
